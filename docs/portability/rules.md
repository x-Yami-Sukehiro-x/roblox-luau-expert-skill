# Portable rules — the single source

Everything in `AGENTS.md`, `.cursor/rules/`, and the GPT files is generated from
this file by `tools/bin/build-portable.mjs`. Edit here, never there.

Each section carries a tier. Tiers are **cumulative and non-overlapping** — a
P1 section continues its P0 counterpart rather than repeating it, so nothing is
stated twice and nothing drifts.

| Tier | Goes to | Why |
|---|---|---|
| **P0** | everything, including the GPT instructions field | That field has a hard character cap, and P0 is what has to survive being the only thing a weak model reads |
| **P1** | the GPT knowledge file, Codex, Cursor | The detail behind P0 |
| **P2** | Codex, Cursor, the GPT knowledge file | Area-specific depth |

`node tools/bin/build-portable.mjs --check` proves the generated files are
current. The build fails if P0 exceeds the GPT instructions cap.
...
## [P0] Non-negotiable runtime rules
...
## [P0] UI: follow the order, do not improvise
...
## [P0] Executor and client-side scripting
...
## [P1] What to decide rather than ask
...
## [P1] Writing for a reader who may not code
...
| "you didn't fix it" | the fix addressed a different layer than the bug |
| "it's broken on my phone" | offset-only sizing |
